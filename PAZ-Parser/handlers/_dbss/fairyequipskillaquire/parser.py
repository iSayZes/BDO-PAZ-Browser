...
def parse_fairyequipskillaquire_records(data: bytes, offset_data: bytes) -> list[dict]:
    ...
    (count,) = struct.unpack_from("<I", data, 0)
    offsets = parse_fairyequipskillaquireoffset_records(offset_data)
    if len(offsets) != count:
        raise ValueError(
            "fairyequipskillaquire count mismatch: "
    ...
    for offset_record in offsets:
        ...
        if data_size != _DATA_SIZE:
            raise ValueError(
                f"fairyequipskillaquire record {source_index} has unexpected size {data_size}"
            )
        if record_start < _HEADER_SIZE or data_offset + data_size > len(data):
            raise ValueError(
                f"fairyequipskillaquire record {source_index} exceeds file size"
            )
