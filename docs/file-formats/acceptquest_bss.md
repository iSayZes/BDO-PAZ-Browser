# `acceptquest.bss` Format

## Purpose

Lists quest IDs in acceptance-related order with two small per-row side fields. The middle two fields in every row form a quest key that matches `allquestlist.bss` exactly once.

Example:

```text
row 6 -> quest 8500 / 4 -> [LoML] Emma's Invitation
```
...
- [completequest.bss](completequest_bss.md) - same row width and quest key scheme, but completion-related order and side fields
...
| Offset  | Type | Field          | Notes                                                                 |
| ------- | ---- | -------------- | --------------------------------------------------------------------- |
| `+0x00` | u16  | unknown_a      | Acceptance-side value; observed range `0..64535`                      |
...
| `+0x06` | u16  | unknown_b      | Acceptance-side value; observed range `0..20314`                      |
...
### Accept Quest Row (8 bytes, repeated `count` times)
...
| --: | --------: | -------------: | -------: | --------: | ------------------------------ |
| 0   | `43008`   | `21312`        | `1`      | `1`       | Lava Fafalun-related quest row |
| 6   | `0`       | `8500`         | `4`      | `0`       | `[LoML] Emma's Invitation`     |
| 7   | `0`       | `12100`        | `1`      | `0`       | `[The Magnus] Nostalgic for Velia` |
| 19  | `0`       | `12101`        | `2`      | `0`       | `[The Magnus] Back to the Abyss` |
...
- The `(quest_chain_id, quest_id)` pair resolves to the same 19,599 packed quest IDs as `allquestlist.bss`. The sets are identical, but row order is completely different from `allquestlist.bss`.
- `acceptquest.bss` and `completequest.bss` have the same size, header, row width, and trailer shape. Both use the middle two `u16` fields as the quest key, but the side fields differ.
...
The two side fields are not confirmed. They likely encode acceptance-related grouping, prerequisites, flags, or routing data, but current local evidence only confirms their boundaries and relationship to quest keys.
