...
Defines one record per pet type/grade combination, covering icon path, species, grade tier, equip-skill slots, and several numeric parameters. Each record represents a specific pet at a specific grade (e.g. "Dog variant #40 at Grade 2"). Companion `petoffset.dbss` enables O(1) lookup by pet ID.

Example:

```text
pet_id: 0xFC05  →  species: 105 (GoldStar)  grade: 4
...
| `petgrade.dbss`  | Optional | `(species, variant) → grade_count` per pet type          |
...
| `+0x0F` | u8   | equip_skill_slots | Number of equip skill slots; = `grade + 1` for regular pets (max 4); Airiss have up to 9 |
...
| `+0x24` | u16      | acquire_type_id | Key into `petequipskillaquire.dbss`; 0 = none; varies by grade |
| `+0x26` | u16      | equip_skill_id  | Pet equip-skill identifier; varies by pet type and grade       |
| `+0x28` | u16      | —               | Always 0; padding                                              |
| `+0x2A` | u32 × 10 | upgrade_table   | 10 values, all 1000000; purpose unknown                        |
| `+0x52` | u8       | —               | Always 0                                                       |
| `+0x53` | u8       | grade_score     | 17 for grade ≥ 3; 16 for grade 2; 11 for grade ≤ 1             |
| `+0x54` | u8       | —               | Always 0                                                       |
...
- `acquire_type_id` values 301–304 appear for regular pets and correspond to grades 0–4 (grade 0 → 301, grade 4 → 304); lower values (1, 2, 3, 4) and mid-range values (101–104, 201–204) appear for specific sub-groups.
...
This byte was initially labeled `dds_variant` under the assumption that it matched the 4-digit icon filename number, but that is incorrect. Cats 0991–0993 all return 0; Cat_0000 returns 45; Dogs show values like 79, 68, 51 that don't match path numbers or grade. The field's actual meaning is unknown.
...
Values: 11 (grade ≤ 1), 16 (grade 2), 17 (grade ≥ 3). Monotonically related to grade but not a simple 1:1 mapping. Possibly a UI display tier, a difficulty tier, or a capacity index.
